...
use App\Models\Erp\SectionOutwardItem;
use Carbon\Carbon;
...
{
    public SectionOutward $sectionOutward;
    public mixed $vid='';
    public mixed $vno = '';
    public mixed $vdate = '';
    ...
    public mixed $sectionOutward_id = '';
    ...
    public function mount($id)
    {
        ...
        if ($id != 0) {
            $this->sectionOutward = SectionOutward::find($id);
            $this->vid = $this->sectionOutward->id;
            $this->vno = $this->sectionOutward->vno;
            $this->vdate = $this->sectionOutward->vdate;
            $this->contact_id = $this->sectionOutward->contact_id;
            $this->contact_name = $this->sectionOutward->contact->vname;
            $this->order_id = $this->sectionOutward->order_id;
            $this->order_name = $this->sectionOutward->order->vname;
            $this->style_id = $this->sectionOutward->style_id;
            $this->style_name = $this->sectionOutward->style->vname;
            $this->total_qty = $this->sectionOutward->total_qty;
            $this->receiver_details = $this->sectionOutward->receiver_details;

            $data = DB::table('section_outward_items')->where('section_outward_id', '=', $id)
                ->join('colours', 'colours.id', '=', 'section_outward_items.colour_id')
                ->join('sizes', 'sizes.id', '=', 'section_outward_items.size_id')
                ->select('section_outward_items.*', 'colours.vname as colour_name', 'sizes.vname as size_name')
                ->get()
                ->transform(function ($data) {
                    return [
                        'section_outward_id' => $data->section_outward_id,
...
//        $this->emit('getfocus');
    ...
    {
        return view('livewire.erp.section-outward.upsert');
    }
}
